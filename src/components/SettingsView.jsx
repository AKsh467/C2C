...
const SettingsView = ({ userProfile, setUserProfile, setNotifications, theme, setTheme, onLogout, notificationsPrefs, setNotificationsPrefs }) => {
    ...
    const handleAvatarChange = (e) => {
        ...
        if (file.size > 2 * 1024 * 1024) { // 2MB limit for local storage / base64 transport
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const reqUrl = `${backendUrl}/api/auth/me`;
            const reqOptions = {
                method: 'PUT',
                headers: {
            ...
            if (response.ok) {
                ...
                if (data.user) {
                    ...
                    setNotifications(prev => [{
                        ...
                        message: 'Your profile has been updated successfully on the server.',
                        time: 'Just now',
                        read: false
                    }, ...prev]);
                }
            ...
            setNotifications(prev => [{
                id: Date.now().toString() + '-settings-err',
                type: 'error',
                title: 'Save Failed',
                message: 'Could not update your profile on the server.',
                time: 'Just now',
                read: false
            }, ...prev]);
        } finally {
    ...
    const handleDeleteAccount = async () => {
        if (!window.confirm('Are you absolutely sure you want to delete your account? This cannot be undone and you will lose all project data.')) return;
            ...
            <div className="flex-col gap-6">
                ...
                <div className="saas-card" style={{ padding: '2rem' }}>
                    ...
                    <div className="flex-col gap-4">
                        {[
                            { id: 'projectUpdates', label: 'Project Updates', desc: 'Receive alerts when milestones are reached or recalculated.', checked: notificationsPrefs?.projectUpdates },
                            { id: 'taskAssignments', label: 'Task Assignments', desc: 'Notify me when I am assigned to a new task.', checked: notificationsPrefs?.taskAssignments },
                            { id: 'teamActivity', label: 'Team Activity', desc: 'Summaries of team chat and comments.', checked: notificationsPrefs?.teamActivity }
                        ].map((pref) => (
                            <div key={pref.id} className="flex items-center justify-between" style={{ padding: '0.5rem 0' }}>
                                <div>
                                ...
                                <div
                                    onClick={() => setNotificationsPrefs(prev => ({ ...prev, [pref.id]: !prev[pref.id] }))}
                                    style={{
                                        width: '44px',
                                        height: '24px',
                                        background: pref.checked ? 'var(--accent-indigo)' : 'var(--panel-border)',
                                        borderRadius: '12px',
                                        position: 'relative',
                                        cursor: 'pointer',
                ...
                <div className="saas-card" style={{ padding: '2rem', borderColor: 'rgba(239, 68, 68, 0.2)', background: 'rgba(239, 68, 68, 0.02)' }}>
                    ...
                    <button className="btn btn-secondary" onClick={handleDeleteAccount} style={{ color: 'var(--danger-color)', borderColor: 'var(--danger-color)' }}>
